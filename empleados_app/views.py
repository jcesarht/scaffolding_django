...
def __update_register(request: Request,id_param: str):
    ...
    data = {}
    try:
        empleado = get_object_or_404(Empleados, emp_id = id_param)
...
def __query(id_param:str = None):
    ...
    try:
        if(id_param == None):
            empleado_model = Empleados.objects.all()
            serializer = EmpleadosSerializer(empleado_model,many=True)
            data = serializer.data                
        else:
            empleado_model = get_object_or_404(Empleados, emp_id = id_param)
            serializer = EmpleadosSerializer(empleado_model)                
            data  = serializer.data
        data = {
            'data': data,
            'http_status': httpstatus.HTTP_200_OK
