...
def __update_register(request: Request,id_param: str):
    ...
    data = {}
    try:
        entity_object = get_object_or_404(Empleados, emp_id = id_param)
        serializer = EmpleadosSerializer(entity_object, data=request.data)
        if(serializer.is_valid()):
            serializer.save()    
            response['status']  = True
            response['message'] = 'Proccess were executed successfully'
...
def __query(id_param:str = None):
    ...
    try:
        if(id_param == None):
            query_set = Empleados.objects.all()
            serializer = EmpleadosSerializer(query_set,many=True)
            data = serializer.data                
        else:
            query_set = get_object_or_404(Empleados, emp_id = id_param)
            serializer = EmpleadosSerializer(query_set)                
            data  = serializer.data
        data = {
            'data': data,
            'http_status': httpstatus.HTTP_200_OK
...
def __delete(id_param:str = None):
    ...
    try:
        ...
        query_set = get_object_or_404(Empleados, emp_id = id_param)
        query_set.delete()
        serialize = EmpleadosSerializer(query_set)
        data = {
            'data': serialize.data,
        }
...
def __verify_request__(request_param:Request):
    ...
    req = request_param
    valid_fields = ["emp_id","fecha_ingreso","nombre",]
